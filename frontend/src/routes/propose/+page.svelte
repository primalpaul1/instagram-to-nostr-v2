<script lang="ts">
  ...
  function selectAllTweets() {
    posts = posts.map(p => ({ ...p, selected: true }));
  }

  function deselectAllTweets() {
    posts = posts.map(p => ({ ...p, selected: false }));
  }
  ...
  <main>
    ...
    {:else if step === 'select'}
      <div class="select-step">
        ...
        {#if (posts.length > 0 && platform !== 'twitter') || articles.length > 0}
          <div class="tabs">
            {#if posts.length > 0 && platform !== 'twitter'}
        ...
        {:else if platform === 'twitter'}
          <div class="toolbar">
            ...
            <div class="toolbar-actions">
              ...
              <button class="text-btn" on:click={selectAllTweets}>Select All</button>
              <button class="text-btn" on:click={deselectAllTweets}>Clear</button>
            </div>
          </div>
